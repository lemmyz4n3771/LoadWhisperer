...
DATA GetData(char* host, DWORD port, char* wresource) {

	DATA data;
	//Convert char* to LPCWSTR
	wchar_t* wWebResource = new wchar_t[4096];
	wchar_t* wHost = new wchar_t[4096];
	MultiByteToWideChar(CP_ACP, 0, wresource, -1, wWebResource, 4096);
	MultiByteToWideChar(CP_ACP, 0, host, -1, wHost, 4096);
	...
	if (hSession)
		...
		std::cout << "[-] WinHttpConnect error " << std::hex << GetLastError();

	// Create an HTTP request handle.
	if (hConnect)
		hRequest = WinHttpOpenRequest(hConnect, L"GET", wWebResource,
			NULL, WINHTTP_NO_REFERER,
			WINHTTP_DEFAULT_ACCEPT_TYPES,
			NULL);
	else
		std::cout << "[-] WinHttpOpenRequest error " << std::hex << GetLastError();
	...
	if (hRequest)
		...
		std::cout << "[-] WinHttpSendRequest error " << std::hex << GetLastError();

	// End the request.
	if (bResults)
		bResults = WinHttpReceiveResponse(hRequest, NULL);
	else
		std::cout << "[-] WinHttpReceiveResponse error " << std::hex << GetLastError();
	...
	if (bResults)
		do
		{
			...
			if (!WinHttpQueryDataAvailable(hRequest, &dwSize))
				std::cout << "[-] WinHttpQueryDataAvailable error " << std::hex << GetLastError();
			...
			else
			{
				...
				if (!WinHttpReadData(hRequest, (LPVOID)pszOutBuffer,
					dwSize, &dwDownloaded))
					std::cout << "[-] WinHttpReadData error " << std::hex << GetLastError();
				else {
		...
		// Report any errors.
		if (!bResults)
			std::cout << "[-] The following error occurred: " << std::hex << GetLastError() << std::endl;
...
int main(int argc, char** argv) {
	...
	for (int i = 0; i < key.len; ++i) {
		std::cout << "0x" << std::setfill('0') << std::setw(2) << std::hex << (0xff & (unsigned int)aesKey[i]);
		if (i < key.len - 1)
			std::cout << ", ";
	}
	std::cout << std::endl;
	...
	for (int i = 0; i < bin.len; ++i) {
		std::cout << "0x" << std::setfill('0') << std::setw(2) << std::hex << (0xff & (unsigned int)payload[i]);
		if (i < bin.len - 1)
			std::cout << ", ";
	}
	std::cout << std::endl;
